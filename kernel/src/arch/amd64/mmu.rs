...
pub mod flags {
    ...
    pub const AP_RW_EL1: usize   = 0;       // Kernel R/W (no USER bit)
    pub const AP_RW_ALL: usize   = 1 << 2;  // User accessible

    pub const UXN: usize         = 1 << 63; // No execute (NX bit)
    pub const PXN: usize         = 1 << 63; // Same as UXN for x86

    pub const PAGE_DEFAULT: usize = PAGE_DESC | AF | ATTR_NORMAL | AP_RW_EL1;
    pub const PAGE_NOEXEC: usize  = PAGE_DESC | AF | ATTR_NORMAL | AP_RW_EL1 | UXN;
    pub const PAGE_DEVICE: usize  = PAGE_DESC | AF | ATTR_DEVICE | AP_RW_EL1 | UXN;
}
