mod dev; mod fat32; mod gpt; mod vfn;

use crate::{
    device::block::BLOCK_DEVICES,
    filesys::{dev::DevFile, fat32::FileAllocTable32, gpt::UEFIPartition, vfn::{FMeta, FType, VirtFNode}},
    printlnk,
    ram::dump_bytes
};
...
pub fn init_filesys() -> Result<(), String> {
    ...
    for (i, part) in UEFIPartition::new(dev.clone())?.get_parts().into_iter().enumerate() {
        ...
        if let Some(fat) = FileAllocTable32::new(partdev.clone()) {
